...
const Login = () => {
  ...
  return (
    ...
    <img src={icon} onClick={()=>navigate('/')}className="arrowicon" alt="back-arrow-icon" title="Go to Home Page"/>
      <h3>Login</h3>
      ...
      <button className={`${isAuthenticated===true?'login auth':'login'}`} type={`${isAuthenticated===true?'button':'submit'}`}>
        Login
      </button>
      </div>
      </form>
